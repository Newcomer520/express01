...
*/
    function ajax(o) {
        var xmlHttp,t=extend({async:true,url:g_URL+'ajax/Tbl2JSON.aspx',dataType:'json'},o);
        if (o.lockAjax) {
        ...
        var k=0;
        try { xmlHttp.srcObject = this; /* try for IE6 got error */
        } catch (e) {}
        var kk=0;
            ...
            if (xmlHttp.readyState==4) {
                ...
                if (xmlHttp.status==200 || xmlHttp.status==0) {
                    ...
                    if (t.dataType=='json' || t.dataType=='jsonColName' || t.dataType=='jsonArray' || t.dataType=='jsonObject') {
                        ...
                        var separator=xmlHttp.getResponseHeader("separator"), QMB=']' /*  "] quotation mark and Brackets */
                            ,lenSep = 3; // length of ],[
                        ...
                        if (o.success) o.success(d,xmlHttp.statusText,xmlHttp);
                    }
...
function appendChild(me,o) {
	...
	for (var key in o) {
		if (key == 'style') {
			...
			for (i=0;i<a.length;i++) { j=a[i].search(':');
				try { 
...
function offset(el) {
	...
	function pick() {var args = arguments,i=0,arg,l = args.length;
...
function colStr(c,pre) {if (c.length==0) return "";var s=c[0];for (var i=1;i<c.length;i++)s+=","+c[i];
...
function getElement(me,o) {
	var i=-1,e=null;
	if (o.by=='innerHTML') {while (++i <me.childNodes.length) if (me.childNodes[i].innerHTML==o.v) {
		e=me.childNodes[i]; break;}}
	else if (o.by == 'className') {while (++i <me.childNodes.length) if (me.childNodes[i].className==o.v) {
		e=me.childNodes[i]; break;}}
	if (e !== null && o.f) o.f(e); 
	return e;
}
      ...
      cellText.onblur = writeBack;
		...
		for (var i=0;i<selectOptions[edFlag-gc_edAC].length;i++) {
            ...
            if (j>0) {
            	...
            	style:'height:38px;' }); /* option to LI */  
            }
            else {
                li=appendChild(sel,{tag:'LI',
          			onmouseover:ACLI_mouseEventHandler,
            	onclick:ACLI_mouseEventHandler,
            	style:'height:20px;',
          		innerHTML:selectOptions[edFlag-gc_edAC][i]}); /* option to LI */  
            }          
    	}
    }
...
,rowsPerPage:10,curPage:0,url:g_URL+'ajax/Tbl2JSON.aspx'}, o );
	var bd=this;
	bd.tBody=t.tBody;	    	   
...
//快速插入一行
,insRow:function(o) { var i,j,k,s='',r=document.createElement('TR'); r.data={};
...
parserEnd */
	...
	else { c.className=  jInfo.clsName.treeC; 
		c.onclick=function() { var e=event,c=this,bd=c.parentNode.parentNode.data.iTbody; 
	  	bd.drillDown(e);
...
vCol.prototype = {	
	...
	,setState :function(handle) {
		var vc = this;
		if (!defined(vc.state) || vc.state=='dragend') { vc.state = 'none';
...
,gotoPage : function(o) {
	...
	ajax({url:g_URL+'ajax/gotoPage.aspx', data:{dbSID:'edwr13',fromRow:fromRow,toRow:toRow,mapPath:tbl.mapPath}
		...
		,error:function(xmlHdr,sts,resText) { alert(sts); 			
			lockAjax(false,tbl.tblObj,false);
		}  
	}); 
...
scrollBody:function (o) {
  ...
  th1r = tb.tHead.tBody.firstChild; 
	...
	wrapper.style.borderRight='none';//width=w+'px';
		...
		divBR.onscroll = function() {var elm=this; event.cancelBubble=true;
		...
		divHR.onscroll = function() {event.cancelBubble=true;
...
init:function(o) {
	...
	var btn= cube.setToolBar({set:'insert',
		btn:[{tag:'INPUT', type:'button',value:'',className:'JTBtnDown',style:'position:absolute;right:5px;left:auto;top:auto;bottom:4px;',
			f:function() {event.cancelBubble=true;
			   var elm=this;
...
onLoad: function(xhdr) { /* when data loaded */
	...
	for (var r=bd.rows[0];r;r=r.nextSibling) {
			...
			if (r.data.layer == cube.treeLayers -1) {
					...
					r.onclick= function() {var r=this,bd=r.parentNode.data.iTbody;
						var e=event;
						e.cancelBubble = true;
...
resetColumn: function(d,xhr) {
    ...
    var isCT=(defined(cube.hCol)), l= cube.vCol.cols.length;    
	  ...
	  if ((isCT || xhr.getResponseHeader('queryLanguage') == 'MDX' ) && cube.hCol.cols.length>0) { 
    	var ttlLen, ttl,deep; 
    	var i,j,d0; 
    	...
    	for (i=0,ttl=1;i<deep;i++) ttl *= cube.hCol.cols[i].value.length;
...
function inputBox_onkeyup() {
	...
	if (event.keyCode==13) { /* enter key */
		...
		if (getData(elm,'before') !== undefined) {elm.value=getData(elm,'before'); elm.className='';
		...
		for (i=0;i<g_options[optIdx].length;i++) {
			...
			if (j>=0) {
            	...
            	if (j>0) {
            		...
            		onclick:ACLI_mouseEventHandler,
            		style:'height:38px;',
            		innerHTML:g_options[optIdx][i].replace('0D0A','<br>')}); /* option to LI */  
            	}
            	else {
                	li=appendChild(sel,{tag:'LI',
                	onmouseover:ACLI_mouseEventHandler,
            		onclick:ACLI_mouseEventHandler,
            		style:'height:20px;',
            		innerHTML:g_options[optIdx][i]}); /* option to LI */  
            	}
			}
	...
	if (defined(o) && defined(o.data)) d=o.data;
 	...
 	for (i=0;i< d.length; i++) {
 		li=appendChild(f.sel,{tag:itemTag,
      	...
      	onmouseover:ACLI_mouseEventHandler,	
      	onclick:ACLI_mouseEventHandler,
      	innerHTML:d[i].replace('0D0A','<br>') }); /* option to LI */ 
...
jAC.prototype = {
 ...
 ,init : function(o) {
  ...
  if (elm.tagName=='INPUT') { l_text=elm.value; elm.value='';
